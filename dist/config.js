    ...
    for (let i = 0; i < argv.length; i += 1) {
        ...
        if (key === "allow-root") {
            ...
            if (Array.isArray(prev))
                ...
                out[key] = [String(prev), String(value)];
            else
...
function bashModeFrom(value) {
    if (value === "off" || value === "safe" || value === "full")
        return value;
    return "safe";
}
function bashTranscriptFrom(value) {
    if (value === "compact" || value === "full")
        return value;
    return "compact";
}
function codexSessionsFrom(value) {
    if (value === "metadata" || value === "read")
        return value;
    if (value === "1" || value === "true" || value === "yes" || value === "on")
        return "metadata";
    return "off";
}
function codexCompatModeFrom(value) {
    if (value === undefined || value === "" || value === "strict")
        return "strict";
    if (value === "off" || value === "safe")
        return value;
    throw new Error("CODEXPRO_CODEX_COMPAT / --codex-compat must be off, safe, or strict.");
}
function bashSessionIdFrom(value) {
...
function writeModeFrom(value) {
    if (value === "off" || value === "handoff" || value === "workspace")
        return value;
    return "workspace";
}
function toolModeFrom(value) {
    if (value === "minimal" || value === "standard" || value === "full")
        return value;
    return "standard";
}
function widgetDomainFrom(value) {
...
export function loadConfig(argv = process.argv.slice(2)) {
    ...
    const requireBashSessionArg = args["require-bash-session"] === true
        ? "true"
    ...
    const toolCardsArg = args["tool-cards"] === true
        ? "true"
    ...
    const requireHttpToken = (!authToken && !allowNoToken) ||
