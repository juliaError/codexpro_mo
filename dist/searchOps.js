...
import { searchWorkspaceStructured } from "./analysis/index.js";
...
function truncateLine(line, max = 400) {
    if (line.length <= max)
        return line;
    return `${line.slice(0, max)}…`;
}
async function runRipgrep(config, guard, workspace, options) {
    ...
    if (options.includeHidden)
        args.push("--hidden");
    for (const glob of config.blockedGlobs)
        args.push("-g", `!${glob}`);
    if (options.glob)
...
async function runNodeSearch(config, guard, workspace, options) {
    ...
    for (const rel of files) {
        if (visibleMatches > options.maxResults)
            break;
...
export async function searchWorkspace(config, guard, workspace, rawOptions) {
    const query = rawOptions.symbol?.toString() || rawOptions.query?.toString() || "";
    if (!query)
        throw new CodexProError("query is required.");
    const options = {
        query,
...
//# sourceMappingURL=searchOps.js.map