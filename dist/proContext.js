...
function parseChangedFiles(status) {
    ...
    for (const rawLine of status.split(/\r?\n/)) {
        ...
        if (!line || line.startsWith("##") || line.startsWith("git unavailable") || line.startsWith("fatal:"))
            continue;
        if (line.length < 4)
            continue;
...
function isLikelyImportantConfig(relPath) {
    const basename = path.basename(relPath);
    return IMPORTANT_ROOT_FILES.includes(relPath) || IMPORTANT_ROOT_FILES.includes(basename);
}
async function existingImportantFiles(guard, workspace) {
    const found = [];
...
async function filesForGlobs(guard, workspace, globs, maxFiles) {
    const out = [];
...
function appendSection(parts, heading, body) {
    parts.push(`## ${heading}\n\n${body.trimEnd()}`);
}
export async function buildProContext(config, guard, workspace, options = {}) {
    ...
    const maxTotalBytes = clamp(options.maxTotalBytes, Math.min(config.maxWriteBytes, 700_000), 20_000, Math.min(config.maxWriteBytes, 2_000_000));
...
export async function exportProContext(config, guard, workspace, options = {}) {
    if (options.includeAiBridge !== false) {
...
//# sourceMappingURL=proContext.js.map