...
function startsWithAllowedPrefix(command) {
...
function isAllowedPackageScript(command) {
    const packageScriptPattern = /^(?:npm|pnpm|yarn|bun)\s+run\s+(?:test|typecheck|lint|build|check)(?::[A-Za-z0-9._-]+)*(?:\s+--\s+[A-Za-z0-9._:= -]+)?$/;
    return packageScriptPattern.test(command);
}
function assertSafeCommand(config, command) {
    ...
    if (!startsWithAllowedPrefix(normalized)) {
        throw new CodexProError(`Command is not in the safe bash allowlist: ${normalized}\n` +
...
function assertBashSession(config, sessionId) {
...
export async function runBash(config, guard, workspace, command, options = {}) {
    if (!command?.trim())
        throw new CodexProError("command is required.");
...
//# sourceMappingURL=bashOps.js.map