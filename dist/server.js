...
const TOOL_CARD_RENDER_TOOL_NAMES = new Set([
...
function usesToolCard(config, name) {
    return config.toolCards && TOOL_CARD_RENDER_TOOL_NAMES.has(name);
}
function descriptorOptionsForConfig(config, name, options) {
...
function assertWriteToolAllowed(config, relPath) {
    ...
    if (config.writeMode === "handoff") {
        throw new CodexProError(`Source writes are disabled because CODEXPRO_WRITE_MODE=handoff. ` +
...
function shouldRegisterTool(config, name) {
    if (config.connectionTest && CONNECTION_TEST_HIDDEN_TOOLS.has(name))
        return false;
    if (name === "codex_bootstrap" || name === "load_skill_resource")
        return config.codexCompatMode !== "off";
...
function serverInstructions(config) {
    const bootstrapInstruction = config.codexCompatMode === "strict"
...
function decodeGitQuotedPath(pathText) {
    ...
    for (let i = 0; i < input.length; i += 1) {
        ...
        if (escaped === undefined)
            throw new CodexProError(`Invalid quoted Git path: ${pathText}`);
        if (/[0-7]/.test(escaped)) {
            let octal = escaped;
...
function applyWorkspacePatch(config, guard, workspace, patch) {
    if (!patch.trim())
        throw new CodexProError("patch is required.");
...
async function writeAgentHandoff(config, guard, workspace, options) {
    ...
    if (!plan)
        throw new CodexProError("plan must not be empty.");
...
export function createCodexProServer(config, serviceState = createCodexProServiceState(config)) {
    ...
    registerCodexTool(config, server, SUPERTOOL_NAME, {
        ...
        description: "Stable wrapper for advanced ChatGPT connector setups. Pass action plus args to call an already-registered CodexPro tool without changing the visible schema; it cannot call tools disabled by the current mode.",
        inputSchema: {
        ...
        if (!handler) {
            throw new CodexProError(`CodexPro action is not available in the current mode: ${action}. ` +
    ...
    registerCodexTool(config, server, "codex_bootstrap", {
        title: "Codex Bootstrap",
        description: "Dynamically assemble the effective Codex AGENTS.md chain, bounded skill manifest, hashes, and configuration compatibility report for a workspace target. No Codex files are copied.",
        inputSchema: {
        ...
        const visibleLayers = result.instructionLayers.map((layer) => includeInstructions ? layer : { ...layer, text: "[omitted by include_instructions=false]" });
        ...
        const text = [
            ...
            includeSkills
                ? visibleSkills.length
                    ? visibleSkills
                        .map((skill) => `- ${skill.name} [${skill.source}] ${skill.active ? "active" : `shadowed by ${skill.shadowedBy}`} — ${skill.skillPath} (${skill.bytes} bytes, ${skill.sha256})`)
                        .join("\n")
    ...
    registerCodexTool(config, server, "load_skill_resource", {
        title: "Load Skill Resource",
        description: "Read one bounded UTF-8 resource by relative path from an already discoverable Codex skill root. Absolute paths, traversal, blocked files, binary data, and symlink escapes are rejected.",
        inputSchema: {
            workspace_id: z.string().optional().describe("Workspace id from open_workspace. Omit to use default workspace."),
    ...
    registerCodexTool(config, server, "codexpro_self_test", {
        title: "CodexPro Self Test",
        description: "Run one controlled, local-only CodexPro diagnostic. It checks modes, expected tools, workspace access, skills, git, safe bash policy, selected-only Pro context, and optional .ai-bridge write/edit probe without touching source files.",
        inputSchema: {
            workspace_id: z.string().optional().describe("Workspace id from open_workspace. Omit to use default workspace."),
            ...
            if (config.writeMode === "off") {
                ...
                try {
                    ...
                    if (!readBack.text.includes("marker: after"))
                        throw new CodexProError("self-test edit marker was not found after edit.");
    ...
    registerCodexTool(config, server, "codexpro_inventory", {
        title: "CodexPro Inventory",
        description: "List CodexPro modes plus discovered skill names and configured MCP server names. Use this early when planning needs local agent capabilities.",
        inputSchema: {
            workspace_id: z.string().optional().describe("Workspace id from open_workspace. Omit to use default workspace."),
    ...
    registerCodexTool(config, server, "open_current_workspace", {
        ...
        description: "Use this once at the start to open the configured default workspace without accepting a path. Do not call open_workspace after this unless switching roots.",
        inputSchema: {
    ...
    registerCodexTool(config, server, "open_workspace", {
        ...
        description: "Open a local project directory as a CodexPro workspace. Returns a workspace_id plus git status, AGENTS.md, and a compact file tree.",
        inputSchema: {
    ...
    registerCodexTool(config, server, "search", {
        ...
        if (result.analysis)
            structured.analysis = result.analysis;
    ...
    if (config.codexSessions !== "off") {
        ...
        if (config.codexSessions === "read") {
            registerCodexTool(config, server, "read_codex_session", {
                ...
                description: "Opt-in, read-only local Codex transcript reader. Requires --codex-sessions read. It selects the newest page by default, returns that page chronologically, and reads the file in blocks instead of loading the full JSONL. Memory use still scales with the largest individual JSONL record scanned.",
                inputSchema: {
