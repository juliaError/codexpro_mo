...
import { CodexProError, displayPath, normalizeRelPath } from "./guard.js";
...
export function textScanByteLimit(config) {
...
export function makeUnifiedDiff(oldText, newText, relPath, maxChars = 60_000) {
    if (oldText === newText) {
    ...
    const out = [`--- a/${relPath}`, `+++ b/${relPath}`, `@@ -${oldStart + 1},${oldEnd - oldStart} +${newStart + 1},${newEnd - newStart} @@`];
    for (let i = oldStart; i < coreOldStart; i += 1)
        out.push(` ${oldLines[i]}`);
    for (let i = coreOldStart; i < coreOldEnd; i += 1)
        out.push(`-${oldLines[i]}`);
    for (let i = coreNewStart; i < coreNewEnd; i += 1)
        out.push(`+${newLines[i]}`);
    for (let i = coreOldEnd; i < oldEnd; i += 1)
        out.push(` ${oldLines[i]}`);
    let diff = out.join("\n");
...
export async function repoTree(config, guard, workspace, options) {
    ...
    const lines = [target.relPath === "." ? "." : `${target.relPath}/`];
    let entries = 0;
    let truncated = false;
    async function walk(absDir, relDir, depth, prefix) {
    ...
    if (truncated)
        lines.push(`...[tree truncated after ${entries} entries]`);
    return { text: lines.join("\n"), entries, truncated };
}
export async function listFiles(guard, workspace, options) {
    const target = guard.resolve(workspace, options.root ?? ".");
    const stat = await fsp.stat(target.absPath);
    const files = [];
    async function addFile(absFile) {
        const rel = displayPath(absFile, workspace.root);
        if (guard.isBlockedRelativePath(rel))
            return;
        if (!options.includeHidden && rel.split("/").some(isHiddenName))
            return;
        if (options.glob && !minimatch(rel, options.glob, { dot: true }))
            return;
        files.push(rel);
    }
    ...
    if (stat.isFile())
        await addFile(target.absPath);
    else
        await walk(target.absPath);
    return files;
}
export async function readTextFile(config, guard, workspace, filePath, options = {}) {
...
export async function writeTextFile(config, guard, workspace, filePath, content, options = {}) {
    ...
    try {
        ...
        if (error instanceof CodexProError && error.message.startsWith("Not a file"))
            throw error;
...
export async function editTextFile(config, guard, workspace, filePath, oldText, newText, options = {}) {
    if (!oldText)
        throw new CodexProError("old_text must not be empty.");
    const resolved = guard.resolve(workspace, filePath, { forWrite: true });
    await guard.assertTextFile(resolved.absPath, Math.max(config.maxWriteBytes, config.maxReadBytes));
...
export async function ensureAiBridge(config, guard, workspace) {
    const files = {
