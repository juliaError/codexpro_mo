...
function cacheKey(workspace, fingerprint, config) {
...
function areasFor(files) {
    ...
    for (const file of files) {
        ...
        current.files += 1;
        if (current.role === "other" && file.role !== "other")
            current.role = file.role;
...
export async function inspectWorkspace(config, guard, workspace) {
    if (!config.analysisEnabled)
        throw new Error("Repository analysis is disabled by CODEXPRO_ANALYSIS=0.");
...
export async function searchWorkspaceStructured(config, guard, workspace, options) {
    ...
    const inScope = (filePath) => !resolvedRoot || filePath === resolvedRoot || filePath.startsWith(`${resolvedRoot}/`);
    const definitionsByPath = new Map();
    for (const symbol of analysis.symbols) {
        const byLine = definitionsByPath.get(symbol.path) ?? new Map();
    ...
    let skippedFiles = 0;
    scan: for (const file of analysis.files) {
        if (file.generated || (!options.includeTests && file.role === "test"))
            continue;
        if (!inScope(file.path) && !(options.includeTests && file.role === "test"))
            continue;
    ...
    if (searchBudgetReached)
        warnings.push("Grouped search reached its configured file or byte limit.");
    if (skippedFiles)
        warnings.push(`Grouped search skipped ${skippedFiles} file${skippedFiles === 1 ? "" : "s"} that changed or became unreadable during analysis.`);
    if (intent === "references" || intent === "impact") {
        const definitionPaths = new Set(analysis.symbols
            .filter((symbol) => symbol.name.toLowerCase() === lowered)
            .map((symbol) => symbol.path));
        for (const relationship of analysis.relationships) {
            if (!definitionPaths.has(relationship.to))
                continue;
            const group = relationship.kind === "tests" ? "tests" : "references";
            if (group === "tests" && !options.includeTests)
                continue;
    ...
    if (candidateLimitReached)
        warnings.push(`Grouped search retained the first ${candidateLimit} candidates before ranking.`);
