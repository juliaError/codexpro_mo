...
const SOURCE_LANGUAGES = new Set(["typescript", "javascript", "python", "go", "rust", "swift", "java", "csharp", "c", "cpp"]);
function importSpecifiers(language, line) {
...
function resolveInternalImport(fromPath, specifier, files) {
...
export async function extractWorkspaceFiles(config, guard, workspace, inventoryFiles) {
    ...
    for (const file of inventoryFiles) {
        if (!SOURCE_LANGUAGES.has(file.language) || file.generated)
            continue;
        ...
        for (let index = 0; index < lines.length; index += 1) {
            ...
            for (const specifier of importSpecifiers(file.language, line)) {
                ...
                if (target && !imports.includes(target))
                    imports.push(target);
            }
        }
