...
import { CodexProError, isSubpath, PathGuard, type Workspace } from "./guard.js";
...
export type CodexInstructionScope = "global" | "project";
export type CodexSkillSource = "project" | "user" | "plugin" | "admin";

export interface CodexInstructionLayer {
  order: number;
...
export interface CodexSkillManifestItem {
  name: string;
  description?: string;
...
export interface CodexConfigCompatibilityReport {
  sources: Array<{ path: string; bytes: number; sha256: string }>;
  applied: Array<{ key: string; value: string | number | string[]; source?: string }>;
...
export interface CodexBootstrapContext {
  ...
  mode: CodexProConfig["codexCompatMode"];
  instructionLayers: CodexInstructionLayer[];
  effectiveInstructions: string;
  skills: CodexSkillManifestItem[];
  configCompatibility: CodexConfigCompatibilityReport;
  contextHash: string;
}

export interface CodexBootstrapResult extends CodexBootstrapContext {
  bootstrapToken: string;
  bootstrappedAt: string;
}

export interface LoadedSkillResource {
  skill: CodexSkillManifestItem;
  resourcePath: string;
  bytes: number;
  sha256: string;
  text: string;
...
interface SkillRecord extends CodexSkillManifestItem {
  absSkillPath: string;
  absSkillRoot: string;
  priority: number;
}
...
function displayPath(absPath: string, config: CodexProConfig, workspace: Workspace): string {
  const codexDir = path.resolve(config.codexDir);
  if (absPath === workspace.root) return "$WORKSPACE";
  if (isSubpath(absPath, workspace.root)) {
    return `$WORKSPACE/${normalizeSlashes(path.relative(workspace.root, absPath))}`;
  }
  if (absPath === codexDir) return "$CODEX_HOME";
...
function regularFileRealpath(filePath: string): string | undefined {
  try {
...
function directoryRealpath(dirPath: string): string | undefined {
  try {
...
async function readBoundedFile(filePath: string, maxBytes: number): Promise<Buffer> {
  ...
  if (!stat.isFile()) throw new CodexProError(`Not a regular file: ${filePath}`);
...
function decodeText(buffer: Buffer, label: string): string {
  if (buffer.includes(0)) throw new CodexProError(`Refusing to read binary content: ${label}`);
  try {
...
function pathMatchesBlockedGlobs(config: CodexProConfig, candidatePath: string): boolean {
  const normalized = normalizeSlashes(candidatePath).replace(/^\.\//, "");
  return config.blockedGlobs.some(
    (glob) =>
...
function safeFallbackNames(
  ...
  for (const raw of values) {
    ...
    if (!names.includes(name) && name !== "AGENTS.override.md" && name !== "AGENTS.md") names.push(name);
  }
  return names.slice(0, 20);
}

async function parseConfigLayer(
  filePath: string,
  config: CodexProConfig,
  workspace: Workspace
): Promise<ParsedConfigLayer | undefined> {
  ...
  if (
    (projectConfig && !isSubpath(real, workspace.root)) ||
  ...
  let fallbackNames: string[] | undefined;
  ...
  if (fallbackMatch) {
    ...
    if (parsed) fallbackNames = safeFallbackNames(parsed, displayPath(real, config, workspace), warnings, config);
    else warnings.push(`Could not parse project_doc_fallback_filenames in ${displayPath(real, config, workspace)}.`);
  }
...
function projectDirectories(workspace: Workspace, targetDirectory: string): string[] {
...
function targetDirectoryFor(
  ...
  try {
    if (!fs.statSync(targetDirectory).isDirectory()) targetDirectory = path.dirname(targetDirectory);
  } catch {
    targetDirectory = path.dirname(targetDirectory);
  }
  const realExisting = directoryRealpath(targetDirectory);
  if (realExisting) targetDirectory = realExisting;
...
async function configCompatibility(
  config: CodexProConfig,
  workspace: Workspace,
  dirs: string[]
): Promise<{ report: CodexConfigCompatibilityReport; fallbackNames: string[]; maxBytes: number }> {
  const candidates = [path.join(config.codexDir, "config.toml"), ...dirs.map((dir) => path.join(dir, ".codex", "config.toml"))];
  const layers: ParsedConfigLayer[] = [];
  ...
  let maxBytesSource: string | undefined;
  const warnings: string[] = [];
...
async function instructionLayer(
  filePath: string,
  scope: CodexInstructionScope,
  order: number,
  maxBytes: number,
  config: CodexProConfig,
  workspace: Workspace
): Promise<CodexInstructionLayer | undefined> {
  const real = regularFileRealpath(filePath);
  if (!real) return undefined;
  if (scope === "project" && !isSubpath(real, workspace.root)) return undefined;
  const codexRoot = directoryRealpath(config.codexDir);
  if (scope === "global" && (!codexRoot || !isSubpath(real, codexRoot))) return undefined;
...
async function discoverInstructions(
  ...
  fallbackNames: string[],
  maxBytes: number
): Promise<CodexInstructionLayer[]> {
  const layers: CodexInstructionLayer[] = [];
...
async function findSkillFiles(root: string, depth: number, out: string[]): Promise<void> {
  if (depth < 0 || out.length >= MAX_SKILLS) return;
...
function skillRoots(config: CodexProConfig, workspace: Workspace, dirs: string[]): Array<{
  root: string;
  anchor: string;
  source: CodexSkillSource;
  priority: number;
  ...
  const roots: Array<{ root: string; anchor: string; source: CodexSkillSource; priority: number; depth: number }> = [
...
async function discoverSkills(
  ...
  for (const candidate of skillRoots(config, workspace, dirs)) {
    ...
    if (!realRoot || !realAnchor || !isSubpath(realRoot, realAnchor)) continue;
    const files: string[] = [];
    ...
    for (const file of files) {
      ...
      if (!realFile || !isSubpath(realFile, realRoot) || seenPaths.has(realFile)) continue;
    ...
    if (records.length >= MAX_SKILLS) {
      throw new CodexProError(
        `Codex skill discovery reached the ${MAX_SKILLS}-skill safety limit. ` +
          "Reduce the configured skill roots before relying on a strict bootstrap."
      );
    }
  }

  const activeByName = new Map<string, SkillRecord>();
  ...
  for (const record of records) {
    ...
    if (!record.active && active) record.shadowedBy = active.skillPath;
  }
  return records.sort(
    (a, b) =>
...
function publicSkill(record: SkillRecord): CodexSkillManifestItem {
  const { absSkillPath: _absSkillPath, absSkillRoot: _absSkillRoot, priority: _priority, ...item } = record;
  return item;
}

function publicContext(context: CodexBootstrapContext): CodexBootstrapContext {
  return {
    ...context,
...
export async function buildCodexBootstrapContext(
  config: CodexProConfig,
  guard: PathGuard,
  workspace: Workspace,
  targetPath = "."
): Promise<CodexBootstrapContext> {
  ...
  const instructionLayers = await discoverInstructions(
    config,
    workspace,
    dirs,
    compatibility.fallbackNames,
    compatibility.maxBytes
  );
...
export class CodexBootstrapRegistry {
  private readonly contextsByWorkspace = new Map<string, Map<string, string>>();
  ...
  async bootstrap(
    ...
    const workspaceContexts = this.contextsByWorkspace.get(workspace.id) ?? new Map<string, string>();
  ...
  async assertFresh(workspace: Workspace, targetPath: string): Promise<CodexBootstrapContext | undefined> {
    if (this.config.codexCompatMode !== "strict") return undefined;
    ...
    if (!stored || ![...stored.values()].includes(current.contextHash)) {
      throw new CodexProError(
        `Strict Codex compatibility requires a fresh codex_bootstrap for ${current.targetDirectory}. ` +
...
function resourcePathIsBlocked(config: CodexProConfig, resourcePath: string): boolean {
  const normalized = normalizeSlashes(resourcePath).replace(/^\.\//, "");
  if (normalized.split("/").some((part) => SENSITIVE_BASENAMES.has(part.toLowerCase()))) return true;
  return pathMatchesBlockedGlobs(config, normalized);
}

export async function loadCodexSkillResource(
  config: CodexProConfig,
  guard: PathGuard,
  workspace: Workspace,
  options: {
  ...
  if (!name) throw new CodexProError("Skill name is required.");
  ...
  if (!matches.length) throw new CodexProError(`Discovered skill not found: ${name}`);
  if (matches.length > 1) {
    throw new CodexProError(
      `Multiple discovered skills named ${name}; pass source and skill_path. ` +
